...
const MOCK_FLAG_KEY = 'portfolio-data-is-mock'; // #4: track ephemeral mock data

/**
 * Safely parse a date value, returning null for invalid dates.
 * Closes #6 — prevents silent NaN dates from cloud/localStorage.
 */
...
export function PortfolioProvider({ children }: { children: React.ReactNode }) {
  ...
  // #5: track pending cloud save when email is not yet confirmed
  ...
  const saveToCloud = useCallback(async (portfolioData: PortfolioData) => {
    ...
    if (!user.email_confirmed_at) {
      ...
      // #5: stash data so we can retry once email is confirmed
  ...
  // #5: retry pending cloud save once the user confirms their email
  useEffect(() => {
    ...
    const loadFromCloud = async () => {
      try {
        ...
        if (rows && rows.length > 0) {
          ...
          // #6: validate dates on cloud load, filter out invalid ones
          ...
          localStorage.setItem(MOCK_FLAG_KEY, 'false'); // #4: mark as real data
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    try {
      // #4: if previous data was mock (ephemeral), clear it and don't reload
      ...
      if (cached) {
        ...
        // #6: validate dates when loading from localStorage
  ...
  const loadMockData = useCallback(() => {
    ...
    // #4: flag as mock so localStorage cache is cleared on next visit
  ...
  const clearData = useCallback(() => {
    ...
    localStorage.removeItem(STORAGE_KEY);
    localStorage.removeItem(MOCK_FLAG_KEY); // #4: clean up mock flag
  }, []);
